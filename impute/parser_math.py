...
def lambda_eval(df, expr):
    return df.eval(expr)


@except_all
def lambda_function(df, expr, kwargs=None):
    ''' Examples of expression that this function must handle:

       '`column 1` + ` column - 2` / `column / 3 ` + `(column z)`'
       '-`column 1` + ` column - 2` / `column / 3 ` + `(column z)`'
       '(`column 1` + ` column - 2`) / `column / 3 `  + `(column z)`'
    '''
    expr = expr.strip()
    if expr.count('`') % 2 != 0:
        raise Exception("Malformed lambda expression - column names must be enclosed with backticks")

    col_alias = compose_num_list(df.shape[1])
    col_orig = list(df.columns)
    # col_orig = [x.strip() for x in col_orig]

    col_expr_clean = re.findall(r'`\s*(.*?)\s*`', expr, re.DOTALL)
    col_expr = re.findall(r'`(.*?)\`', expr, re.DOTALL)

    col_map = dict(zip(col_expr, col_alias))
    # col_clean_map = dict(zip(col_orig, col_alias))

    for col, col_clean in zip(col_expr, col_expr_clean):
        if col_clean not in col_orig:
            raise Exception(f"Malformed lambda expression - column `{col}` not found")
        expr = expr.replace("`" + col + "`", col_map[col])
    df.columns = col_alias
    ...
    def __init__(self, string, vars = None):
        ...
        for constant in _CONSTANTS.keys():
            if self.vars.get(constant) != None:
                raise Exception("Cannot redefine the value of " + vars)
